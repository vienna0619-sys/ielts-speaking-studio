...
- 浏览器层只使用“精确 locale + 人工核验名称 allow-list”的 Microsoft/Google/Apple 声音，并排除常见 novelty/eSpeak 声音。指定声音不可用时只允许回退到同性别的已验证声音；不会静默使用系统默认声音或异性声音。
...
- 高频音量分析、嘴型、练习教练的眨眼/呼吸/视线和音频上下文计数由单一共享 `requestAnimationFrame` 调度器管理；开发环境或 `?debugPerformance=1` 可查看 FPS、嘴型值、循环数和 AudioContext 数。
...
| 变量 | 默认值 | 用途 |
| --- | --- | --- |
| `OPENAI_TRANSCRIBE_MODEL` | `gpt-4o-mini-transcribe` | 英语录音转写 |
| `OPENAI_TTS_MODEL` | `gpt-4o-mini-tts` | Speech API 自然考官语音 |
| `OPENAI_TTS_VOICE_GB_FEMALE` | `marin` | 可选英式女声音色覆盖 |
| `OPENAI_TTS_VOICE_GB_MALE` | `cedar` | 可选英式男声音色覆盖 |
| `OPENAI_TTS_VOICE_US_FEMALE` | `marin` | 可选北美女声音色覆盖 |
| `OPENAI_TEXT_MODEL` | `gpt-5.6-terra` | Responses API 严格结构化复盘报告 |
| `AI_ALLOWED_ORIGINS` | GitHub Pages 与 Sites 正式域名 | 服务端 CORS allow-list |
| `VITE_AI_API_BASE_URL` | Sites 正式域名 | 静态前端连接的服务端地址 |

实现使用官方 `openai` SDK。转写使用 `gpt-4o-mini-transcribe`；语音使用 `gpt-4o-mini-tts`；评分使用 Responses API Structured Outputs 与 Zod 校验，默认 `gpt-5.6-terra`，并设置 `store: false`。连接诊断只发送一个极小的合法请求，不上传整场录音。
...
- GitHub Pages `https://vienna0619-sys.github.io/ielts-speaking-studio/` 继续提供静态前端，构建时通过 `VITE_AI_API_BASE_URL` 连接 Sites。
- Sites `https://ielts-speaking-studio.vienna0619.chatgpt.site` 运行同一源码的全栈版本以及 `/api/ai` 健康检查、诊断、TTS、转写与评分路由。
- `OPENAI_API_KEY` 只配置在 Sites（或其他受信任服务端）的运行环境；不能放进 GitHub Pages、`VITE_` 或 `NEXT_PUBLIC_` 变量。
- 服务端只允许明确配置的前端 origin，评分请求使用 requestId/幂等键，并区分认证、额度、限流、模型、超时、空转写与结构校验错误。
